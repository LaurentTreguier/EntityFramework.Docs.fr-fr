...
> [!TIP]  
> Vous pouvez [afficher et télécharger l’exemple de cet article](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/SimpleLogging) sur GitHub.
...
> [!TIP]
> EF Core s’intègre également à [Microsoft. extensions. Logging](/aspnet/core/fundamentals/logging), ce qui nécessite davantage de configuration, mais il est souvent plus approprié pour la journalisation dans les applications de production.
...
Par défaut, les timestamnps sont conçus pour la consommation locale pendant le débogage. Utilisez `DbContextLoggerOptions.DefaultWithUtcTime` plutôt pour utiliser des horodatages UTC indépendantes de la culture, mais conservez tout le reste. Par exemple :
...
Il est parfois utile d’avoir une seule ligne par message de journal. Cette option peut être activée par `DbContextLoggerOptions.SingleLine` . Par exemple :
